    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are a speech-to-text transcription engine. Transcribe the audio exactly as spoken in English. Output ONLY the transcribed text, nothing else. No labels, no quotes, no commentary. If the audio is silent or unintelligible, respond with exactly: [NO_SPEECH]",
          },
          {
            role: "user",
            content: [
